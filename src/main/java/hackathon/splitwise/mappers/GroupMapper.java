...
public class GroupMapper {
    ...
    public static List<GroupDto> mapToGroupDtoList(List<GroupDetailsEntity> groupEntities, List<UserGroupMappingEntity> userGroupMappingEntities) {
        ...
        return groupEntities.stream()
                .map(groupEntity -> mapToGroupDto(groupEntity, groupIdToAmountPaidMap.getOrDefault(groupEntity.getId(), 0.0)))
                .collect(Collectors.toList());
    }

    // Mapper method to convert GroupDetailsEntity to GroupDto, with amountPaid
    public static GroupDto mapToGroupDto(GroupDetailsEntity groupEntity, Double amountPaid) {
        return new GroupDto(
                groupEntity.getId(),
                groupEntity.getName(),
                groupEntity.getType(),
